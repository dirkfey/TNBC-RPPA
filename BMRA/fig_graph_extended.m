...
mysvgInteractions = [
    ...
    "AKT-p38"   "AKT_p"            "p38MAPK_T180_"
    "GSK3B-CHK1" "GSK3B_S9_"       "CHK1_S345_"
    "AMPK-PTEN"  "AMPK_T172_"      "PTEN"
    "AKT-PI3K"   "AKT_p"           "PI3KalPha"
    "FAK-PDK"    "FAK_Y925_"       "PDK1_S241_"
    ];

mysvgInteractions_diff = [
    "AKT-p38"   "AKT_p"            "p38MAPK_T180_"
    "GSK3B-CHK1" "GSK3B_S9_"       "CHK1_S345_"
    "AMPK-PTEN"  "AMPK_T172_"      "PTEN"
    "AKT-PI3K"   "AKT_p"           "PI3KalPha"
...
xDoc = xmlread('./graph_NW_2_base_extended.svg');
xDoc = svg_colorInteractions(xDoc, mysvgInteractions, myNW, myNetwork, [0 0 0]);
xmlwrite('../figs/graph_NW_2extended_canonical_tmp.svg',xDoc)


myNW = myConsNW_sens;
myNW(abs(myNW)<0.4)=0;
xDoc = xmlread('./graph_NW_2_base_extended_v2.svg');
xDoc = svg_colorInteractions(xDoc, mysvgInteractions, myNW, myNetwork);
xDoc = svg_colorInteractions(xDoc, mysvgInteractions_diff, myConsNW_sens, myNetwork);
xmlwrite('../figs/graph_NW_2extended_sensitive_tmp.svg',xDoc)

myNW = myCOnsNW_resi;
myNW(abs(myNW)<0.4)=0;
xDoc = xmlread('./graph_NW_2_base_extended_v2.svg');
xDoc = svg_colorInteractions(xDoc, mysvgInteractions, myNW, myNetwork);
xDoc = svg_colorInteractions(xDoc, mysvgInteractions_diff, myCOnsNW_resi, myNetwork);
for i = 1:length(mysvgInteractions_diff)
    svglabel = mysvgInteractions_diff{i,1};
    xDoc = svg_setPathWidthByLabel(xDoc, svglabel ,0.5);
end
xmlwrite('../figs/graph_NW_2extended_resistant_tmp.svg',xDoc)
